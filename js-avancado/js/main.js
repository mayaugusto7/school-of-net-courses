...
function formatDesc(desc) {
    ...
    str = str.charAt(0).toUpperCase() + str.slice(1); // pega a partir da letra na posicao 1
    return str;
}
...
function addData() {
    ...
    list.unshift({ "desc": desc, "amount": amount, "value": value }); //adiciona no topo da lista unshift
...
function deleteData(id) {
    ...
    if (confirm('Delete this item?')) {
        if (id === list.length - 1) {
            list.pop(); // remove o ultimo registro da lista
        } else if (id === 0) {
            list.shift(); // remove o primeiro elemento da lista
        } else {

            // remove elemento do meio da lista pelo index
